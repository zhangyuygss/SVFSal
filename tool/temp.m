...
for idx = 1:length(imgList)
    ...
    str_std = sprintf('Top5 score std: %f', std(value_category(1:5)));
    str_thre = sprintf('Object number: %d', sum(value_category > 0.05));
    ...
    figure; 
    subplot(1,2,1); imshow(cur_heat_map); title(str_std);
    subplot(1,2,2); imshow(img); title(str_thre);
end
