...
class Net(nn.Module):
	...
	def forward(self, data = None, **variables):
		...
		for module in [module for module in self.children() if not all(name in variables for name in module.caffe_output_variable_names)]:
			# print('Layer:{} Out:{} Pre:{} Type:{}'.format(module.caffe_layer_name, module.caffe_output_variable_names,
			# 						module.caffe_input_variable_names, module.caffe_layer_type))
...
modules = dict(
	...
	Reshape = lambda param: Reshape(param),
	Sigmoid = lambda param: nn.Sigmoid(),
	Concat = lambda param: Concat(param),
...
class Deconvolution(nn.ConvTranspose2d):
	def __init__(self, param):
		super(Deconvolution, self).__init__(first_or(param,'group',1), param['num_output'], 
		        kernel_size = first_or(param, 'kernel_size', 1), stride = first_or(param, 'stride', 1),
		        padding = first_or(param, 'pad', 0), dilation = first_or(param, 'dilation', 1),
		        groups = first_or(param, 'group', 1), bias=first_or(param, 'bias_term', True))
	...
	def forward(self, x):
		if self.weight.numel() == 0 and self.bias.numel() == 0:
			...
			super(Deconvolution, self).__init__(x.size(1), self.out_channels, kernel_size = self.kernel_size,
...
class Reshape(nn.Module):
    def __init__(self, param):
        ...
        self.shape = param['shape']['dim']

    def forward(self, x):
        orig_shape = x.size()
        new_shape = [orig_shape[i] if self.shape[i] == 0 else self.shape[i] for i in range(len(self.shape))]
        return x.view(*new_shape).contiguous()

    def __repr__(self):
        return 'Reshape(dims=%s)' % (self.shape)
...
def init_weight_bias(self, weight = None, bias = None, requires_grad = []):
	...
	else:  # init bias with 0
		self.bias = nn.Parameter(torch.zeros(weight.size(1)).type_as(self.bias),
